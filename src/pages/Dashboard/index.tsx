...
/*   const formatMoney = (value: number, category: string): string => {
    if (category === 'outcome') {
      return ` - R$ ${Intl.NumberFormat().format(value)},00`;
    }
    return ` R$ ${Intl.NumberFormat().format(value)},00`;
  }; */
  ...
  useEffect(() => {
    async function loadTransactions(): Promise<void> {
      api.get('/transactions').then(resp => {
        ...
        const teste = respTransactions.map((r: Transaction) => {
          r.formattedValue = formatValue(r.value);
          return r;
        });
